...
class Tile(object):
   ...
   def get_manhattan_dist(self, board_width):
      ...
      return abs(self.col - goal_col) + abs(self.row - goal_row)
   ...
   def get_manhattan_dist(self):
      ...
      for i in range(self.width):
         for j in range(self.width):
            ...
            tile.row = i
            tile.col = j
            total_dist += tile.get_manhattan_dist(self.width)
      return total_dist

   def get_manhattan_delta(self, blank, target):
      prev_man_dist = target.get_manhattan_dist(self.width)
      target.row = blank.row
      target.col = blank.col
      new_man_dist = target.get_manhattan_dist(self.width)
      return (new_man_dist - prev_man_dist)
...
def get_fringe_states(puzzle):
   ...
   for move in puzzle.moves:
      target = get_target(blank, move)
      if puzzle.in_bounds(target):
         manhattan_delta = puzzle.get_manhattan_delta(blank, target)
         next_puzzle = create_next_puzzle(puzzle, blank, move, target, manhattan_delta)
         fringe_states.append(next_puzzle)

   return fringe_states

def get_target(blank, move):
   print(blank.col)
...
def create_next_puzzle(puzzle, blank, move, target, delta_manhattan):
   ...
   next_puzzle.manhattan_dist = puzzle.manhattan_dist + delta_manhattan
